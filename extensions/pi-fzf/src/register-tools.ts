...
export function registerTools(pi: ExtensionAPI, deps: ToolRegistrationDeps): void {
	...
	if (!isFeaturePersistedEnabled("registerBuiltInOverrides")) {
		// Skip read/grep registration — Pi native tools remain active.
	} else {
	...
	} // end registerBuiltInOverrides guard
