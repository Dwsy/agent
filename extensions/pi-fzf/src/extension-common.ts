...
export type FeatureKey = "autocomplete" | "builtInToolEnhancements" | "registerBuiltInOverrides" | "agentTools" | "statusUI";
...
export const FEATURE_DEFINITIONS: FeatureDefinition[] = [
	...
	{ id: "registerBuiltInOverrides", label: "Register built-in overrides", description: "Override read/grep tools (off = use Pi native)" },
...
/**
 * Synchronous check against persisted feature state.
 * Used at extension load time (before async restore runs).
 * Returns true if feature is enabled, false if disabled.
 * Defaults to true when no persisted state exists.
 */
export function isFeaturePersistedEnabled(feature: FeatureKey): boolean {
	try {
		if (!existsSync(GLOBAL_FEATURES_PATH)) return true;
		const raw = readFileSync(GLOBAL_FEATURES_PATH, "utf8");
		const parsed = JSON.parse(raw) as FeatureState | undefined;
		if (!Array.isArray(parsed?.enabledFeatures)) return true;
		return parsed.enabledFeatures.includes(feature);
	} catch {
		return true;
	}
}
