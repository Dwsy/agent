...
function modeStatusText(ctx: ExtensionContext, mode: ThinkingStepsMode): string {
	const modeText = mode === "collapsed" ? "collapsed" : mode === "expanded" ? "expanded" : "summary";
	return `${ctx.ui.theme.fg("muted", "thinking:")} ${ctx.ui.theme.fg("accent", modeText)}`;
}
...
function refreshThinkingUI(ctx: ExtensionContext, announce = false): void {
	if (!ctx.hasUI) return;
	(ctx.ui as any).setHiddenThinkingLabel(DEFAULT_HIDDEN_LABEL);
...
export default function thinkingStepsExtension(pi: ExtensionAPI): void {
	...
	pi.registerCommand("thinking-steps", {
		description: "Switch thinking view: collapsed, summary, expanded",
