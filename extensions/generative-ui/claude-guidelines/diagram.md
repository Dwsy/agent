...
| id | Use when |
|---|---|
| `flow-steps` | 2–5 linear process steps, no branches. Quiet vertical rail. |
| `flow-mermaid` | Branches, pipelines, state-ish flows. Neutral theme:base palette. |
| `architecture-cards` | Module map / system layers. Hybrid with a tiny Mermaid overview if needed. |

**How to use:** pick the closest id → call `visualize_read_me` again with `templates: ["flow-steps"]` (or multiple ids) to load full HTML → paste into `show_widget` → swap copy/data.
Default response is **catalog only** (token-light). Pass `templates: ["all"]` only when you need every body.
